...
namespace Sudoku.Engine.Tests.Solvers
{
  internal class StraightLineRemovesPencilMarksSolverTests
  {
    ...
    public void GivenBoard01SolveResultsCorrectHavingSingleStraightLineValueInGroupColumnSolver(string gameBoardInput, string solvedGameOutput)
    {
      ...
      var group0LeftColumnPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 0)).GetColumnCells(GroupColumnPosition.Left).GetPencilMarks();
      ...
      var group0RightColumnPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 0)).GetColumnCells(GroupColumnPosition.Right).GetPencilMarks();
      ...
      var group3LeftColumnPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 27)).GetColumnCells(GroupColumnPosition.Left).GetPencilMarks();
      ...
      var group3RightColumnPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 27)).GetColumnCells(GroupColumnPosition.Right).GetPencilMarks();
      ...
      var group6LeftColumnPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 54)).GetColumnCells(GroupColumnPosition.Left).GetPencilMarks();
      ...
      var group6MiddleColumnPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 54)).GetColumnCells(GroupColumnPosition.Middle).GetPencilMarks();
    ...
    public void GivenBoard01SolveResultsCorrectHavingSingleStraightLineValueInGroupRowSolver(string gameBoardInput, string solvedGameOutput)
    {
      ...
      var group0TopRowPencilMarks = result.GetGroupBy(result.Cells.First(x => x.Index == 0)).GetRowCells(GroupRowPosition.Top).GetPencilMarks();
      ...
      var group0BottomRow = result.GetGroupBy(result.Cells.First(x => x.Index == 0)).GetRowCells(GroupRowPosition.Bottom).GetPencilMarks();
      var expectedGroup0BottomRow = new List<int>() { 7, 8, 9 };
      Assert.That(group0BottomRow, Is.EquivalentTo(expectedGroup0BottomRow));

      var group1TopRow = result.GetGroupBy(result.Cells.First(x => x.Index == 3)).GetRowCells(GroupRowPosition.Top).GetPencilMarks();
      var expectedGroup1TopRow = new List<int>() { 7, 8, 9 };
      Assert.That(group1TopRow, Is.EquivalentTo(expectedGroup1TopRow));

      var group1BottomRow = result.GetGroupBy(result.Cells.First(x => x.Index == 3)).GetRowCells(GroupRowPosition.Bottom).GetPencilMarks();
      var expectedGroup1BottomRow = new List<int>() { 1, 2, 3, 7, 8, 9 };
      Assert.That(group1BottomRow, Is.EquivalentTo(expectedGroup1BottomRow));

      var group2MiddleRow = result.GetGroupBy(result.Cells.First(x => x.Index == 6)).GetRowCells(GroupRowPosition.Middle).GetPencilMarks();
      var expectedGroup2MiddleRow = new List<int>() { 7, 8, 9 };
      Assert.That(group2MiddleRow, Is.EquivalentTo(expectedGroup2MiddleRow));

      var group2BottomRow = result.GetGroupBy(result.Cells.First(x => x.Index == 6)).GetRowCells(GroupRowPosition.Bottom).GetPencilMarks();
