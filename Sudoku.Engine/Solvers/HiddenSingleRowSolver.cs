...
namespace Sudoku.Engine.Solvers
{
  ...
  /// that is the ONLY pencil mark across the group, row, and column for this cell.
  ...
  /// </summary>
  public class HiddenSingleRowSolver : ISolver
  {
    ...
    {
      return SolverExecutionOrder.HiddenSingleRow;
    }
    ...
    private bool IsHiddenSinglePencilMark(GameCell cell, int cellPencilMark)
    {
      ...
      var rowMinusCell = _GameBoard.GetRowBy(cell).Cells.Except(exceptList).SelectMany(x => x.PencilMarks);
      var cellsPencilMarksGroupedWithCount =
        rowMinusCell.GroupBy(val => val).Where(numGroup => numGroup.Key == cellPencilMark);
